...
//-- AUTH --
export const storeUser = functions.auth.user().onCreate(event => {
	...
	return firestore.collection('users').doc(data.uid).delete()
		.then(() => {
			return Promise.all([
				...
				deleteCollection(firestore.collection(`users/${data.uid}/connections`), 10), // delete connections subCollection of the user
				Promise.all([ // delete connectionHeleprs and otherUser/connections where user == data.uid
					...
					results[0].docs.forEach((snap, idx) => {
						...
						updates.push(
							firestore.doc(`users/${userId}/connections/${data.uid}`)
							.delete()
						)
					});
 ...
 */
export const sendNewConnectionNotification = functions.firestore.document('connectionRequests/{requestId}').onCreate(event => {
 ...
 */
export const sendConnectionEstablishedNotification = functions.firestore.document('connectionHelpers/{connectionId}').onCreate(event => {
	const record = event.data.data();
 ...
 */
export const sendLocationResponseNotification = functions.firestore.document('locations/{locationId}').onCreate(event => {
...
function sendMessageAndCleanupTokens(tokenCollectionSnap: Array<any>, payload) {
	const tokens = [];
...
app.post('/createConnectionRequest', (req, resp) => {
	...
	.then((addResult) => {
		...
		connectionRequest.created = (connectionRequest.created as Date).getTime();
...
app.post('/disconnect', (req, resp) => {
	const initiator = (req as any).userid;
