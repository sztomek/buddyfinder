...
class LoadLocationUseCase @Inject constructor(private val appSchedulers: AppSchedulers, private val datasource: IDatasource) {

    fun execute(action: Action.GetLocationByIdAction): Single<Result> {
        return datasource.getLocation(action.locationId)
