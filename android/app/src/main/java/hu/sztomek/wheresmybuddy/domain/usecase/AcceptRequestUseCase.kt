...
class AcceptRequestUseCase @Inject constructor(private val appSchedulers: AppSchedulers, private val datasource: IDatasource) {

    fun execute(action: Action.AcceptRequestAction): Single<Result> {
        return datasource.acceptConnectionRequest(action.requestId)
